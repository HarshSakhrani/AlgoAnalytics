...
scheduler=torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='min',factor=0.15,patience=2,min_lr=1e-7,verbose=True)
...
def train_model(model,patience,epochs):
  ...
  return model,avgTrainLoss,avgValidLoss,avgTrainAcc,avgValidAcc,lrList

model,avgTrainLoss,avgValidLoss,avgTrainAcc,avgValidAcc,lrList=train_model(model,3,10)

plt.plot(range(1,11),lrList)
plt.xlabel("Epochs")
plt.ylabel("LR")
plt.savefig('preTrainingLrList.png', bbox_inches='tight')
plt.close()
...
scheduler=torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='min',factor=0.15,patience=3,min_lr=1e-7,verbose=True)
...
model,avgTrainLoss,avgValidLoss,avgTrainAcc,avgValidAcc,lrList=train_model(model,3,15,avgTrainLoss,avgValidLoss,avgTrainAcc,avgValidAcc)

plt.plot(range(1,len(avgTrainLoss)+1),lrList)
plt.xlabel("Epochs")
plt.ylabel("LR")
plt.savefig('fineTuningLrList.png', bbox_inches='tight')
plt.close()
