...
class CustomizedVGG(nn.Module):
  def __init__(self,inChannels=1,embSize=512,noClasses=1251):
    ...
    self.encoderLayer = nn.TransformerEncoderLayer(d_model=512, nhead=8)
    self.transformerEncoder = nn.TransformerEncoder(self.encoderLayer, num_layers=3)
  ...
  def forward(self,input,preTrainingFlag=0):
    ...
    H=convOutput.shape[-1]
    W=convOutput.shape[-2]
    ...
    convOutput = convOutput.flatten(2).permute(2, 0, 1)

    convOutput = self.transformerEncoder(pos + convOutput)

    convOutput = convOutput.permute(1, 2, 0)
    convOutput = torch.reshape(convOutput, ((convOutput.shape)[0], 512 * 180))
